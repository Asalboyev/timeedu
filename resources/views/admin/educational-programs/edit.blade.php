                        ...
                        <form method="post" action="{{ route($route_name . '.update', $educationalProgram->id) }}" enctype="multipart/form-data" id="add">
                            ...
                            <div class="row">
                                ...
                                <div class="tab-content" id="myTabContent">
                                    @foreach($langs as $lang)
                                        <div class="tab-pane mt-3 fade {{ $loop->first ? 'show active' : '' }}" id="{{ $lang->code }}" role="tabpanel" aria-labelledby="{{ $lang->code }}-tab">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control @error('secon_name.'.$lang->code) is-invalid @enderror" name="secon_name[{{ $lang->code }}]" value="{{ old('secon_name.'.$lang->code) ?? ($educationalProgram->secon_name[$lang->code] ?? '') }}" id="title" placeholder="Secon name...">
                                                @error('secon_name.'.$lang->code)
                                                <span class="invalid-feedback" role="alert">
                                                    <strong>{{ $message }}</strong>
                                                </span>
                                                @enderror
                                            ...
                                            <div class="form-group">
                                                ...
                                                <textarea name="secon_description[{{ $lang->code }}]" id="desc" cols="30" rows="10" class="form-control @error('secon_description.'.$lang->code) is-invalid @enderror ckeditor" name="secon_description[{{ $lang->code }}]" placeholder="Secon description...">{{ old('secon_description.'.$lang->code)?? $educationalProgram->secon_description[$lang->code] ?? '' }}</textarea>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <input type="text" class="form-control @error('title.'.$lang->code) is-invalid @enderror" name="name[{{ $lang->code }}]" value="{{ old('name.'.$lang->code) ?? ($educationalProgram->name[$lang->code] ?? '') }}" id="title" placeholder="Name...">
                                    ...
                                    @endforeach
                                        @if ($educationalProgram->id != 1 && $item->id != 2) <!-- 1 va 2 idni yashirish -->
                                        <div class="form-group">
                                            ...
                                            <select class="form-select @error('parent_id') is-invalid @enderror" id="parent_id" name="parent_id">
                                                ...
                                                @foreach ($educationalPrograms as $key => $item)
                                                    <option value="{{ $item->id }}" {{ old('parent_id') ?? $educationalProgram->parent_id == $item->id ? 'selected' : '' }}>{{ $item->name[$main_lang->code]?? '' }}</option>
                                                @endforeach
                                            </select>
                                            @error('parent_id')
                                            <span class="invalid-feedback" role="alert">
